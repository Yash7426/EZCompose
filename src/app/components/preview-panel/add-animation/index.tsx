...
type SubOptions = keyof AnimationItem ;
type AnimationParamTypes ={
    animationDuration: number;
    animationDelay: number;
    animationRepeat: number;
    animationName: string;
    subAnimationList:AnimationOption[];
...
export default function AnimationOptionsPanel(props:Allow) {
    ...
    let [animParam, setAnimParam] = useState<AnimationParamTypes>({ 
                ...
                <div className='animItems'>
                    {
                        animations.map((el, idx) => {
                            return (<div className='animItem' key={el.itemName} data-animationname={el.animationName} data-animationnum={idx} onClick={(e:React.MouseEvent<HTMLDivElement, MouseEvent>) => {
                                //anim name
                                let aniN = ((e.currentTarget.hasAttribute("data-animationname")) ? e.currentTarget.getAttribute("data-animationname") : e.currentTarget.closest("[data-animationname]")?.getAttribute("data-animationname") )as string ;
                                const animationNum=(e.currentTarget.hasAttribute("data-animationnum"));
                                let subOptions=Number( animationNum ? e.currentTarget.getAttribute("data-animationnum") : e.currentTarget.closest("[data-animationnum]")?.getAttribute("data-animationnum")) ;

                                let animDirection = "";
                                if (!isNaN(subOptions) && subOptions && !animations[subOptions].hasDirection) {
                                    animDirection = animations[subOptions].extraOptions[0].addon;
                                }

                                // setAnimParam((prev)=>({ ...prev, animationName: aniN, subAnimationList: animations[subOptions].extraOptions, animDirection }));
                                if( aniN!==undefined && aniN!==null  && typeof subOptions==="number" && typeof animDirection==="string"){
                                    setAnimParam((prev)=>({ ...prev, animationName: aniN, subAnimationList: animations[subOptions].extraOptions, animDirection }));
                                }
                                // setAnimParam((prev)=>({ ...prev, animationName: aniN, subAnimationList: animations[subOptions].extraOptions, animDirection }));
            ...
            <div className='advOptions' draggable="false" style={(!panelMode) ? {} : { display: "none" }}>
                ...
                <div className='animItems advanceWidth' draggable="false">
                    ...
                    <div className='anim_option_elem'>
                                ...
                                <>
                                    ...
                                    <div className='anim_slider_input' data-parentslide>
                                        <select className='subAnimSelector' onChange={(e) => {
                                            ...
                                            (document.querySelector("[data-path=\"" + props.currentlyActive.current + ",\"]") as HTMLElement).style.animationIterationCount = itr as string;
                        ...
                        <div className='anim_slider_input' data-parentslide>
                            <input onChange={(e) => {
                                setAnimParam((prev)=>({ ...prev, animationDuration: Number(e.target.value) }))
                                elemAnimUpdate();
