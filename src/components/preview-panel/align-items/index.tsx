...
const AlignItems:React.FC<AlignItemsProps>=({closePanel,currentlyActive})=> {
...
type IflexOptions={
    ...
    const prevFlexOptions = () => {
        let node = document.querySelector("[data-path=\"" + currentlyActive.current + ",\"]") as HTMLElement;
if(node){

    node.style.alignItems = flexAlignOptions.alignItems as unknown as string;
    node.style.justifyContent = flexAlignOptions.justifyContent as unknown as string;
    ...
    node.classList.add("alignOptionsPrev");
        ...
        if (prevTimerHelper.current){ 
            ...
            prevTimerHelper.current = setTimeout(() => {
                ...
                document.querySelector("[data-path=\"" + currentlyActive.current + ",\"]")?.classList.remove("alignOptionsPrev")
            }, 2000)
            ...
            <div className='wdAlignInner'>
                ...
                {HorizontalAlign.map((option, index) => {
                    return (
                        ...
                        onClick={() => setFlexAlignOptions((prev)=>({ ...prev, alignItems: option.alignItems }))}>
                ...
                {VerticalAlign.map((option, index) => {
                    return (
                        ...
                        key={index}
                        className='align-option'
                        onClick={() => setFlexAlignOptions((prev)=>({ ...prev, justifyContent: option.justifyContent }))}>
                            <img src={option.imgSrc} />
                            <span>{option.label}</span>
                        </div>
                    )
