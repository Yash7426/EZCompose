...
export default function PaddingSettings(props:Allow) {
...
type IelPadding = {
    ...
    const [elPadding, setElPadding] = useState<IelPadding> ({
        lockedAll: true,
        paddingTop: 0,
        paddingLeft: 0,
        paddingBottom: 0,
    ...
    let previewPaddingHelper = useRef<Allow>(null);
...
if(node){
    ...
    previewPaddingHelper.current = setTimeout(() => {
        let node2 = document.querySelector("[data-path=\"" + props.currentlyActive.current + ",\"]");
if(node2){
    node2.classList.remove("paddingChPre")
}
    }, 2000);
}
    }
