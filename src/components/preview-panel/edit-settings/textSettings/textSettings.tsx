...
export type TextSetting = {
    ...
    shadows: Partial<Ishadow>[]
}

export default function TextSettings(props: Allow) {
    ...
    const [textSetting, setTextSetting] = useState<TextSetting>({
        panelMode: 0,
        fontFamily: "Poppins",
        fontSize: 1,
        fontWeight: 500,
    ...
    const genTextShadow = () => {
        ...
        for (let i of textSetting.shadows) {
            let __rgb_shadow ;
            if(i.color)
            __rgb_shadow= hexToRgb(i.color);
        if(__rgb_shadow && i.opacity)
    ...
    const prevTextDecor = () => {
        ...
        node.style.fontWeight = textSetting.fontWeight.toString();
