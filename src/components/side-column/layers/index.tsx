...
export default function ElementLayer() {
    ...
    const collapseParentList = (e: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
        e.stopPropagation()
        ...
        if (node) {
            ...
            if (!node && e.currentTarget.hasAttribute("data-spannode")) {
                ...
                node = e.currentTarget.closest("li")?.querySelector(".subNode") as HTMLLIElement;
            }
    ...
    const MultiLayers = ({datapath,e}:{datapath:string;e:IdesignState[]}) => {

        return (
            <>
                {
                    e.map((el, i) => {
                        return (<GenerateLayer element={el} datapath={datapath + i + ','} key={datapath + i + ','} >{el.elementType}</GenerateLayer>)
                    })
                }
            </>
        )
    ...
    const selectLayerNode = ( e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
        e.stopPropagation()
        let __active_node = e.currentTarget.closest("[data-path-layer]")?.getAttribute("data-path-layer");
        if(__active_node)
    ...
    const highlightNode = ( e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
        ...
        if (e.currentTarget.hasAttribute("data-path-layer")) {
            ...
            __el = e.currentTarget.closest("[data-path-layer]")?.getAttribute("data-path-layer");
        }
    ...
    const removeHighlightNode = (e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
        e.stopPropagation()
        let __el;
        if (e.currentTarget.hasAttribute("data-path-layer")) {
            __el = e.currentTarget.getAttribute("data-path-layer");
    ...
    const duplicateLayerNode = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        e.stopPropagation()
        let __active_node = e.currentTarget.closest("[data-path-layer]")?.getAttribute("data-path-layer");
...
if(__active_node){
    ...
    if (__active_node.split(",").length < 2 && pageDesignState.design?.elements) {
    ...
    const GenerateLayer = ({datapath,element,children}:{datapath:string,element:IdesignState,children?:React.ReactNode}) => {
        let e = element;
                 ...
                 <span data-spannode
                  ...
                  <div
                   ...
                   onMouseEnter={highlightNode}>
                    ...
                    <button className='elementNodeSelector'
                    onClick={selectLayerNode}>
                        <i className="las la-mouse-pointer"></i>
                    </button>

                    <button
                    className='duplicateLayer'
                    onClick={duplicateLayerNode}>
